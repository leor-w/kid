...
func (cli *Client) Scan(cursor uint64, match string, count int64) *redis.ScanCmd {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	return cli.Client.Scan(ctx, cursor, match, count)
}

func (cli *Client) ScanType(cursor uint64, match string, count int64, keyType string) *redis.ScanCmd {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	return cli.Client.ScanType(ctx, cursor, match, count, keyType)
}

func (cli *Client) SScan(key string, cursor uint64, match string, count int64) *redis.ScanCmd {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	return cli.Client.SScan(ctx, key, cursor, match, count)
}

func (cli *Client) HScan(key string, cursor uint64, match string, count int64) *redis.ScanCmd {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	return cli.Client.HScan(ctx, key, cursor, match, count)
}

func (cli *Client) ZScan(key string, cursor uint64, match string, count int64) *redis.ScanCmd {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func New(opts ...Option) *Client {
	...
	cli.Client = redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", cli.options.Addr, cli.options.Port),
